...
async function stepCreateSDS() {
  ...
  async function stepJoinSDS() {
  try {
    //options.channel = $("#channel").val();
    options.channel = "channel1";
    options.uid = Number($("#uid").val());
    options.token = await agoraGetAppData(options);
    await join()
    setOptionsToLocal(options)
  ...
  async function stepSubscribeSDS() {
  const uid = $("#remote-uid").val()
  const user = remoteUsers[uid]
  if (!user) {
    return message.error(`User:${uid} not found!`)
...
$("#step-publish-sds").click(async function (e) {

  $("#step-publish-sds").attr("disabled", true);
  $("#step-leave-sds").attr("disabled", false)
  stepCreateSDS();
  stepJoinSDS();
  await createTrackAndPublish()
  addSuccessIcon("#step-publish")
...
$("#step-leave-sds").click(async function (e) {
  ...
  $("#join").attr("disabled", false);
  $("#leave").attr("disabled", true);
  $("#step-leave-sds").attr("disabled", true);
  $("#step-join").attr("disabled", true);
  $("#step-publish-sds").attr("disabled", false);
  $("#step-subscribe").attr("disabled", true);
  $("#mirror-check").attr("disabled", true);
  $("#step-create").attr("disabled", false);
  $("#remote-uid").val("")
